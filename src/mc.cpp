...
mc::check()
{
  ...
  for(int i = 0; i<global_cfg.threads; i++)
  {
    ...
    id->data = this;
    id->id = i;
    //! start each thread  using the thread_wrapper function
...
bool
mc::check_properties(node *n)
{
  if (n->is_visited())
    return true;

  for (map<string, query*>::iterator i = properties.begin();
      i != properties.end(); i++)
  {
    if (!i->second->check(n))
    {
      ...
      if (!global_cfg.check_local)
        ...
        cout << "[Backtrace unavailable in local checking mode]" << endl;
...
{
  stringstream out;
  rinfo("mc::report") << "[q: " << workqueue.size() << " / g: " << g->size()
      << "] @ " << step;
  g->dump_dot_forward();
  return out.str();
}
...
mc::step(int id)
{
  ...
  {
    rinfo("mc::step") << "Interrupted!  Reached maximum steps: "
        << global_cfg.max_steps << endl;
    cout << report(steps) << endl;
    return false;
  }
  ...
  pthread_mutex_lock( &mcmutex );
#endif
//! Enable to retrieve the results
if (results!=NULL)
  (*results)[step] = n->bg->get_root(0)->to_string();
...
void
mc::add_property(string s, query *q)
{
  properties[s] = q;
}

void
mc::match_mark_delete(match *m)
{
  assert(m != NULL);
