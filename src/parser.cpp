...
check_warn()
{
  fprintf(
      stderr,
      "Error: your model file doesn't have a '%%check' directive, so it won't do anything! Exiting...\n");
}
...
parser_next_char()
{
  ...
  if (strncmp("exit", g_inputbuffer, 4) == 0
      || strncmp("quit", g_inputbuffer, 4) == 0)
  {
    if (global_cfg.function_mode)
      return 0;
    else
  ...
  if (strncmp("check", g_inputbuffer, 5) == 0
      || strncmp("check;", g_inputbuffer, 6) == 0
      || strncmp("%check", g_inputbuffer, 6) == 0
      || strncmp("%check;", g_inputbuffer, 7) == 0)
  {
    return EOF;
...
parser::init(char *file)
{
  ...
  if (tty && file == NULL)
  {
    print_version();
    cout
        << "Hint: remember to enter 'check' once you have finished entering your model!"
        << endl;
    g_fp = NULL;
    using_history();
  }
...
void
add_result(parsenode *p)
{
  g_parsetree.push_back(p);
}

term *
parser::bg_mknode(prefixnode *p)
{
  if (DEBUG)
...
set<term *>
parser::bg_collapse(parallelnode *p)
{
...
set<term *>
parser::bg_collapse(parsenode *p)
{
...
list<term *>
parser::bg_rcollapse(regionnode *p)
{
...
list<term *>
parser::bg_rcollapse(parsenode *p)
{
...
term *
parser::bg_mknode(parallelnode *p)
{
  if (DEBUG)
...
term *
parser::bg_mknode(regionnode *p)
{
  if (DEBUG)
    fprintf(stderr, "BUG: parser::bg_mknode(regionnode): ");
...
term *
parser::bg_mknode(nilnode *p)
{
  if (DEBUG)
    fprintf(stderr, "BUG: parser::bg_mknode(nilnode): ");

  return new nil();
}

term *
parser::bg_mknode(numnode *p)
{
  if (DEBUG)
    fprintf(stderr, "BUG: parser::bg_mknode(numnode): ");

  return new num(p->data);
}

vector<name>
parser::bg_names(seqnode *p)
{
...
term *
parser::bg_mknode(controlnode *p)
{
  if (p == NULL)
    return new nil();
...
parser::bg_mknode(parsenode *p)
{
  ...
  switch (p->type)
  {
    ...
    default:
      ...
      cerr << p->to_string() << endl;
      if (global_cfg.function_mode)
        throw runtime_error("Malformed term structure");
      else
        exit(1);
      break;
  }
...
query_val *
parser::mk_queryval(parsenode *p)
{
  if (p == NULL)
  {
    rerror("parser::mk_queryval") << "Invalid value in property term" << endl;
    if (global_cfg.function_mode)
      throw runtime_error("Invalid value in property term");
    else
      exit(1);
  }
...
list<query_val *>
parser::mk_queryargs(parsenode *p)
{
  list<query_val *> vals;
...
parser::mk_query(parsenode *p)
{
  switch (p->type)
  {
    ...
    default:
      cerr << "parser::mk_query(): Invalid syntax in query: " << p->to_string()
          << endl;
      if (global_cfg.function_mode)
        throw runtime_error("Invalid syntax in query");
      else
        exit(1);
...
int
parser::parse_function(const std::string& model, results_map& results)
{
  //! the input buffer is the input model
  char * buffer = new char[model.size() + 1];
  std::copy(model.begin(), model.end(), buffer);
  buffer[model.size()] = '\0'; // don't forget the terminating 0
  //
  g_inputbuffer = buffer;
  g_inputbuffer_len=model.size() + 1;
  yyparse();
  bigraph *b = parser::finish();
  g_has_check = true;
  driver::check_function(b, results);
      ...
      case NODE_REACTION:
        {
          ...
          reactionrule *rr = new reactionrule(bg_mknode(t->redex),
                                              bg_mknode(t->reactum));
